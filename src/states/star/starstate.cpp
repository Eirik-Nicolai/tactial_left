...
void StarState::init(TacticalGame *ge)
{
    ...
    handler->register_input(
        INPUT_TYPE::middle_mouse_PRESSED,
        std::make_shared<InputDebugWrapper>(std::make_shared<PanInputStart>()));
    handler->register_input(
        INPUT_TYPE::middle_mouse_RELEASED,
        std::make_shared<InputDebugWrapper>(std::make_shared<PanInputEnd>()));
    handler->register_input(
        INPUT_TYPE::middle_mouse_HELD,
        std::make_shared<InputDebugWrapper>(std::make_shared<PanInputUpdate>()));

    handler->register_input(
        INPUT_TYPE::middle_mouse_SCROLLDOWN,
        std::make_shared<InputDebugWrapper>(std::make_shared<ScrollUpInput>()));

    handler->register_input(
        INPUT_TYPE::middle_mouse_SCROLLUP,
        std::make_shared<InputDebugWrapper>(std::make_shared<ScrollDownInput>()));
...
void StarState::cleanup(TacticalGame *ge) { LOG_FUNC }

void StarState::enter(TacticalGame *ge) { LOG_FUNC }
void StarState::exit(TacticalGame *ge) { LOG_FUNC }

void StarState::pause(TacticalGame *ge) { LOG_FUNC }
void StarState::resume(TacticalGame *ge) { LOG_FUNC }
void StarState::handle_input(TacticalGame *ge)
{
    ...
    //     (ge->GetMousePos() - tv->GetTileOffset()) / tv->GetWorldScale(); for(auto [ent,
    //     pos, circular] : reg.view<Pos, SizeCirc, Tag::Hoverable,
    //     Tag::Selectable>().each()) {
    //         if(isInside(pos.coordinates.x, pos.coordinates.y, circular.r, mouse_pos.x,
    //         mouse_pos.y))
    //         {
...
void StarState::update(TacticalGame *ge)
{
    ...
    auto mouse_pos = tv->ScaleToWorld(pos_mouse) +
                     tv->GetWorldOffset(); // (ge->GetMousePos() - tv->GetTileOffset()) /
                                           // tv->GetWorldScale();
...
void StarStateSelected::cleanup(TacticalGame *ge) { LOG_FUNC }
...
void StarStateSelected::pause(TacticalGame *ge) { LOG_FUNC }
void StarStateSelected::resume(TacticalGame *ge) { LOG_FUNC }
...
void StarStateSelected::update(TacticalGame *ge)
{
    ...
    if (pointofinterest != entt::null) {
        ...
        tv->SetWorldOffset(coordinates - (tv->ScaleToWorld({ge->ScreenWidth() / 2.f,
                                                            ge->ScreenHeight() / 2.f})));
    }
}
