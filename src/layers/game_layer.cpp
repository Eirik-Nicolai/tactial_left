...
void GameLayer::update(TacticalGame *ge) {
    for (auto &state : std::views::reverse(m_current_states)) {
        ...
        // HACK to use olc::TV, this should use the event system
        state->handle_input(ge);
    }
}
...
bool GameLayer::key_pressed(TacticalGame *, KeyPressedEvent &event) {
  return true;
}

bool GameLayer::mouse_button_released(TacticalGame *,
                                      MouseButtonReleasedEvent &event) {
    
    return true; 
}
bool GameLayer::mouse_button_pressed(TacticalGame *,
